...
export default function VendorPortal() {
    ...
    const confirmDelivery = async (id: string) => {
        try {
            ...
            { transactionId: id, newStatus: 'Confirmed', notes: 'Vendor confirmed delivery' },
            { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }});
    ...
    return (
        <div className="p-6">
            ...
            <p className="mb-6 text-gray-600">View your outgoing transactions and manage statuses.</p>
                        ...
                        {transactions.map(tx => (
                            <tr key={tx.transactionId}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    {tx.status === 'Sent to Accounts' && (
                                        <button onClick={() => confirmDelivery(tx.transactionId)} className="text-indigo-600 hover:text-indigo-900 mr-4">Confirm Receipt</button>
                                    )}
                                </td>
                            </tr>
                        ))}
