...
import { UpdateFormDataDocument } from "@web/generated/graphql-operations";
// import { useMutation } from "@web/graphql/clients/hooks";
...
const useFormBuilder = (props: useFormBuilderProps) => {
  ...
  const deleteControl = (controlId: string, containerId: string) => {
    console.log(controlId, containerId);
    const newState = formLayoutComponents.slice();
    ...
    if (formContainerIndex !== -1) {
      ...
      const newFormContainer = {
        ...formContainer,
        children: formContainer.children.filter(
          (cont) => cont.id !== controlId,
        ),
      };

      newState[formContainerIndex] = newFormContainer;
      console.log(newFormContainer.children);
  ...
  const checkIfControlsInContainer = () => {
    for (let i = 0; i < formLayoutComponents.length; i++) {
      const componentChildren = formLayoutComponents[i].children;
  ...
  const publishForm = () => {
    ...
    const currentTemplate: TemplateType = JSON.parse(
  ...
  const saveForm = async (
    onHandleUpdateFormData: (templates: TemplateType) => Promise<any>,
  ) => {
    if (formLayoutComponents.length === 0) {
      showModalStrip("danger", "Form cannot be empty", 5000);
      return;
    ...
    const template = await dispatch(saveTemplate(currentTemplate)).unwrap();
    onHandleUpdateFormData(template).then((resolvedvalue) => {
