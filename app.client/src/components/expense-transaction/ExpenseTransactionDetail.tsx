...
import type { ExpenseTransactionDetailVM } from "@/api/models/ExpenseTransactionDetailVM";
...
export function ExpenseTransactionDetail({
  ...
  const [detail, setDetail] = useState<ExpenseTransactionDetailVM | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchDetail = async () => {
    setLoading(true);
    try {
      const res = await ExpenseTransactionService.getExpenseTransactionById(transactionId, "1");
      if (res.success) {
        ...
        <div className="relative group overflow-hidden bg-white/60 dark:bg-card/40 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2.5rem] p-8 transition-all hover:border-primary/20 shadow-lg">
          ...
          <div className="space-y-1">
            <p className="text-4xl font-black text-foreground tracking-tighter tabular-nums">
              ...
              {detail.expenseAmount?.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </p>
        ...
        <div className="relative group overflow-hidden bg-white/60 dark:bg-card/40 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2.5rem] p-8 transition-all hover:border-emerald-500/20 shadow-lg">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm font-black text-foreground uppercase tracking-tight">{detail.expenseTypeName || "General Expense"}</p>
            </div>
            <div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm font-black text-foreground uppercase tracking-tight">
                {detail.dateOfExpense ? new Date(detail.dateOfExpense).toLocaleDateString(undefined, { dateStyle: 'long' }) : "N/A"}
              </p>
            </div>
            <div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white/40 dark:bg-card/30 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2.5rem] p-8 sm:p-10 space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div className="space-y-4">
                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-2">
                  <Store className="h-3 w-3 text-primary/60" /> Vendor Information
                </p>
                <div className="flex items-center gap-4 bg-muted/10 rounded-2xl p-4 border border-white/5">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase text-foreground">{detail.vendorId ? "Certified Vendor" : "Direct Pay"}</p>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-60">ID: {detail.vendorId || "Self-Managed"}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 bg-muted/10 rounded-2xl p-4 border border-white/5">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase text-foreground">{detail.expenseName || "General Project Spending"}</p>
