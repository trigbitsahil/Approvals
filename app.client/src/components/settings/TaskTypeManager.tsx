...
interface TaskTypeManagerProps {
  ...
  onTypesUpdated: (newTypes?: TaskTypeListVM[]) => void;
  projectTypes: TaskTypeListVM[];
}
...
const SortableTaskTypeItem: React.FC<SortableItemProps> = ({ type, onClick, onDelete }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    transition,
    isDragging,
  } = useSortable({ id: type.taskTypeId! });
  ...
  return (
    ...
    >
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          onDelete(type.taskTypeId!);
        }}
        className="text-muted-foreground hover:text-destructive  group-hover:opacity-100 p-1 rounded hover:bg-background transition-all shrink-0"
      >
...
export const TaskTypeManager: React.FC<TaskTypeManagerProps> = ({
  ...
  const [workspaceTypes, setWorkspaceTypes] = useState<TaskTypeListVM[]>([]);
  ...
  const [editingType, setEditingType] = useState<TaskTypeListVM | null>(null);
  const [editName, setEditName] = useState("");
  const [editColor, setEditColor] = useState("");
  const [isUpdating, setIsUpdating] = useState(false);

  // Drag and Drop State
  const [orderedTypes, setOrderedTypes] = useState<TaskTypeListVM[]>([]);
  ...
  // Fetch workspace types when entering "add" mode
  useEffect(() => {
    if (modalState === "add") {
      TaskTypeService.taskTypeGet("1")
        .then((res) => {
          if (res.success && res.data) {
            // Filter out types that are already in the project
            const existingIds = projectTypes.map(t => t.taskTypeId);
            setWorkspaceTypes(res.data.filter(t => !existingIds.includes(t.taskTypeId)));
          }
        })
  ...
  const handleCreateType = async () => {
    ...
    setIsCreating(true);
    try {
      const response = await TaskTypeService.taskTypePost("1", {
        name: newTypeName,
        code: newTypeName.replace(/\s+/g, "_"), // Simple slug
        category: "Project",
      ...
      if (response.success) {
        toast.success(`Task type "${newTypeName}" created!`);
        onTypesUpdated();
        setModalState("edit");
        setNewTypeName("");
      } else {
        toast.error(response.message || "Failed to create task type.");
      }
    } catch (error) {
  ...
  const handleUpdateType = async () => {
    ...
    setIsUpdating(true);
    try {
      const response = await TaskTypeService.taskTypePut("1", {
        taskTypeId: editingType.taskTypeId,
        name: editName,
        code: editingType.code, // Keep original code or generate new one
      ...
      if (response.success) {
        ...
        toast.error(response.message || "Failed to update task type.");
      }
    } catch (error) {
  ...
  const handleAddSelectedTypes = async () => {
    ...
    try {
      for (const typeId of selectedWorkspaceTypes) {
        const typeToAdd = workspaceTypes.find(t => t.taskTypeId === typeId);
        if (typeToAdd) {
          await TaskTypeService.taskTypePost("1", {
            name: typeToAdd.name,
            code: typeToAdd.code,
            category: "Project",
            categoryId: projectId,
      ...
      toast.success("Task types added to project!");
      onTypesUpdated();
      setModalState("edit");
    } catch (error) {
  ...
  const startEditDetail = (type: TaskTypeListVM) => {
    setEditingType(type);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      setOrderedTypes((items) => {
        const oldIndex = items.findIndex((i) => i.taskTypeId === active.id);
        const newIndex = items.findIndex((i) => i.taskTypeId === over.id);
  ...
  const handleDeleteType = async (typeId: string) => {
    try {
      const response = await TaskTypeService.deleteTaskType(typeId, "1");
      if ((response as any).success) {
        toast.success("Task type deleted successfully");
        onTypesUpdated();
        setOrderedTypes(prev => prev.filter(t => t.taskTypeId !== typeId));
      } else {
        toast.error("Failed to delete task type");
      }
    } catch (error) {
      toast.error("An error occurred while deleting task type");
    }
  };
      ...
      <DialogContent className="sm:max-w-[425px] p-0 overflow-hidden bg-background border-border">
        ...
        {modalState === "edit" && (
          <div className="p-6 space-y-6">
            <DialogHeader>
              <DialogTitle className="text-xl font-semibold">Edit types</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-1">
              ...
              >
                <SortableContext
                  items={orderedTypes.map(t => t.taskTypeId!)}
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <DialogTitle className="text-xl font-semibold">Add task type</DialogTitle>
              </div>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="space-y-1 max-h-[300px] overflow-y-auto">
              <p className="text-[10px] uppercase font-bold text-muted-foreground mb-2 px-1">Available task types in the workspace</p>
              {filteredWorkspaceTypes.length > 0 ? (
                filteredWorkspaceTypes.map((type) => (
                  ...
                  >
                    ...
                    <Checkbox
                      checked={selectedWorkspaceTypes.includes(type.taskTypeId!)}
                      onCheckedChange={(checked) => {
                        setSelectedWorkspaceTypes(prev =>
                          checked
                            ? [...prev, type.taskTypeId!]
                            : prev.filter(id => id !== type.taskTypeId)
                        );
                      }}
                    />
                  </label>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              >
                {isAddingSelected ? "Adding..." : "Add task types"}
              </Button>
            </div>
          </div>
        )}
        ...
        {modalState === "create" && (
          <div className="p-6 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <DialogTitle className="text-xl font-semibold">Create task type</DialogTitle>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              <Button onClick={handleCreateType} disabled={isCreating || !newTypeName.trim()}>
                {isCreating ? "Creating..." : "Create task type"}
              </Button>
            </div>
          </div>
        )}
        ...
        {modalState === "edit-type" && (
          <div className="p-6 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <DialogTitle className="text-xl font-semibold">Edit task type</DialogTitle>
            </div>
