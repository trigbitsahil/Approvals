...
function useWatch(props) {
	...
	const [value, updateValue] = import_react.useState(control._getWatch(name, _defaultValue.current));
	useIsomorphicLayoutEffect(() => control._subscribe({
		name,
...
function getDirtyFieldsFromDefaultValues(data, formValues, dirtyFieldsFromValues) {
	const isParentNodeArray = Array.isArray(data);
	if (isObject(data) || isParentNodeArray) for (const key in data) if (Array.isArray(data[key]) || isObject(data[key]) && !objectHasFunction(data[key])) if (isUndefined(formValues) || isPrimitive(dirtyFieldsFromValues[key])) dirtyFieldsFromValues[key] = Array.isArray(data[key]) ? markFieldsDirty(data[key], []) : { ...markFieldsDirty(data[key]) };
...
function createFormControl(props = {}) {
		...
		for (const name in fields) {
			...
			if (field) {
				...
				if (_f) {
					...
					if (isPromiseFunction && _proxyFormState.validatingFields) _updateIsValidating([name], true);
		...
		if (field) {
			...
			if (_options.resolver) {
				...
				if (isFieldValueUpdated) {
					const errorLookupResult = schemaErrorLookup(errors, _fields, schemaErrorLookup(_formState.errors, _fields, name).name || name);
	...
	const watch = (name, defaultValue) => isFunction(name) ? _subjects.state.subscribe({ next: (payload) => name(_getWatch(void 0, defaultValue), payload) }) : _getWatch(name, defaultValue, true);
...
function useFieldArray(props) {
	...
	const _fieldIds = import_react.useRef(fields);
	const _name = import_react.useRef(name);
