...
//#region node_modules/react-resizable-panels/dist/react-resizable-panels.browser.development.js
...
var useId = import_react["useId".toString()];
...
function PanelWithForwardedRef({ children, className: classNameFromProps = "", collapsedSize, collapsible, defaultSize, forwardedRef, id: idFromProps, maxSize, minSize, onCollapse, onExpand, onResize, order, style: styleFromProps, tagName: Type = "div", ...rest }) {
	...
	if (!(0, import_react.useRef)({ didLogMissingDefaultSizeWarning: false }).current.didLogMissingDefaultSizeWarning);
...
function getPanelGroupElement(id, rootElement = document) {
	if (isHTMLElement(rootElement) && rootElement.dataset.panelGroupId == id) return rootElement;
...
function useWindowSplitterPanelGroupBehavior({ committedValuesRef, eagerValuesRef, groupId, layout, panelDataArray, panelGroupElement, setLayout }) {
	const devWarningsRef = (0, import_react.useRef)({ didWarnAboutMissingResizeHandle: false });
	useIsomorphicLayoutEffect(() => {
...
function validatePanelConstraints({ panelConstraints: panelConstraintsArray, panelId, panelIndex }) {
	{
		...
		if (defaultSize != null) {
			...
			else if (defaultSize < minSize && (!collapsible || defaultSize !== collapsedSize)) warnings.push("default size should not be less than min size");
	...
	(0, import_react.useEffect)(() => {
		{
			...
			if (!didLogIdAndOrderWarning) {
				...
				if (prevPanelIds.length > 0 && !areEqual(prevPanelIds, panelIds)) {
...
function getIntersectingRectangle(rectOne, rectTwo, strict) {
	if (!intersects(rectOne, rectTwo, strict)) return {
		x: 0,
		y: 0,
		width: 0,
		height: 0
