//#region node_modules/export-from-json/dist/esm/utils.js
...
//#endregion
//#region node_modules/export-from-json/dist/esm/processors.js
...
//#endregion
//#region node_modules/export-from-json/dist/esm/converters.js
...
function _createFieldsMapper(fields) {
	...
	return function(item) {
		if (isArray(item)) return item.map(function(i) {
			return getEntries(i).reduce(function(map, _a) {
				var _b = __read(_a, 2), key = _b[0], value = _b[1];
				if (key in mapper) map[mapper[key]] = value;
				return map;
			}, Object.create(null));
		}).filter(function(i) {
...
function _createTableEntries(tableMap, beforeTableEncode) {
	if (beforeTableEncode === void 0) beforeTableEncode = function(i) {
		return i;
	};
...
function encloser(value, delimiter) {
	var enclosingCharacter = new RegExp("".concat(delimiter, "|\"|\n")).test(value) ? "\"" : "";
...
function createCSVData(data, options) {
	...
	var tableEntries = _createTableEntries(_createTableMap(data), beforeTableEncode);
	return tableEntries.map(function(_a) {
		return _a.fieldName;
	}).join(delimiter) + "\r\n" + tableEntries.map(function(_a) {
		return _a.fieldValues;
	}).map(function(column) {
		return column.map(function(value) {
			return encloser(value, delimiter);
		});
	}).reduce(function(mergedColumn, column) {
...
function _renderTableHTMLText(data, beforeTableEncode) {
	...
	var rows = tableEntries.map(function(_a) {
		return _a.fieldValues;
	}).map(function(column) {
		return column.map(function(value) {
...
function createXMLData(data) {
	return "<?xml version=\"1.0\" encoding=\"utf-8\"?><!DOCTYPE base>\n".concat(_renderXML(data, "base"), "\n");
}
function _renderXML(data, tagName, arrayElementTag, spaces) {
	...
	if (data === null || data === void 0) return "".concat(indentSpaces, "<").concat(tag, " />");
...
//#endregion
//#region node_modules/export-from-json/dist/esm/types.js
var exportTypes = {
	txt: "txt",
	css: "css",
	html: "html",
...
//#endregion
//#region node_modules/export-from-json/dist/esm/exportFromJSON.js
function exportFromJSON(_a) {
	...
	if (typeof data === "string") switch (exportType) {
		case "txt":
		case "css":
		case "html": return processor(data, exportType, normalizeFileName(fileName, extension !== null && extension !== void 0 ? extension : exportType, fileNameFormatter));
		default: throw new Error(MESSAGE_FIELD_INVALID);
	}
	var safeData = _createFieldsMapper(fields)(_prepareData(data));
	var JSONData = _createJSONData(safeData, replacer, space);
	switch (exportType) {
		case "txt":
		case "css":
		case "html": return processor(JSONData, exportType, normalizeFileName(fileName, extension !== null && extension !== void 0 ? extension : exportType, fileNameFormatter));
		case "json": return processor(JSONData, exportType, normalizeFileName(fileName, extension !== null && extension !== void 0 ? extension : "json", fileNameFormatter));
		case "csv":
		...
		default: throw new Error(MESSAGE_UNKNOWN_EXPORT_TYPE);
	}
}
exportFromJSON.types = exportTypes;
exportFromJSON.processors = { downloadFile };
//#endregion
//#region node_modules/export-from-json/dist/esm/index.js
var esm_default = exportFromJSON;
//#endregion
